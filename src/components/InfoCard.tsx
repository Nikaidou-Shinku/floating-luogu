...
import { AM_BADGE_STYLE, AM_RADIUS_STYLE, LG_BG, LG_FG, LG_FL } from "../styles/luoguStyles";
...
const CardBottom = (props: { name: string, svgType: number }) => {
  return (
    <div style={STAT_BOTTOM_STYLE}>
      <svg viewBox="0 0 512 512" style={{width: 14, height: 14, verticalAlign: "middle"}}>{cardButtomIconLust[props.svgType]}</svg>&nbsp;{props.name}
    </div>
  )
}
...
const getCCFLevelBadge = (value: number) => {
  // Font Awesome Free 6.0.0-beta3 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free (Icons: CC BY 4.0, Fonts: SIL OFL 1.1, Code: MIT License) Copyright 2021 Fonticons, Inc.
  return (
    <svg viewBox="0 0 512 512" style={
      $CSS([
        LG_FL(getCCFLevelColor(value)),
        {
          width: 20,
          height: 20,
          position: "absolute",
...
export const InfoCard = (props: { info: UserInfo, currUID: number }) => {
  const userInfo = props.info;
  const cUID = props.currUID;
  ...
  const hasRelationship = (userInfo.userRelationship != undefined)
   && (userInfo.uid != cUID);
  return (
      ...
      <div style={CARD_CONTAINER_STYLE}>
        <div style={CARD_HEADER_STYLE}>
          <div style={{width: 80}}>
            ...
            {(userInfo.ccfLevel >= 3) && getCCFLevelBadge(userInfo.ccfLevel)}
          </div>
          <div style={{flex: 1}}>
            <div style={
              $CSS([
